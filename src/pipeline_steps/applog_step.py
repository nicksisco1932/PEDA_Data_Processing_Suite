# PURPOSE: Locate and consolidate TDC log files into TDC Sessions/applog/.
...
def _walk_candidates(root: Path) -> List[Path]:
    if not root.exists() or not root.is_dir():
        return []
    return [p for p in root.rglob("*") if _is_candidate(p)]
...
def _find_tdc_log_with_meta(case_root: Path) -> Tuple[Optional[Path], int, Optional[str]]:
...
def find_tdc_log(case_root: Path) -> Optional[Path]:
    log_path, _, _ = _find_tdc_log_with_meta(case_root)
    return log_path


def install_tdc_log(case_root: Path, case_id: str) -> Dict[str, Any]:
    log_path, candidate_count, selection_reason = _find_tdc_log_with_meta(case_root)
    if log_path is None:
        return {
            "status": "skipped",
            "reason": "no_log_found",
    ...
    dest_dir = case_root / "TDC Sessions" / "applog"
    dest_path = dest_dir / f"{case_id} {log_path.name}"
