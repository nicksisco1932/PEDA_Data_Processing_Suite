#!/usr/bin/env python3
r"""
master_run.py  (v2.3.2-compat+autoid)

- Backward-compatible flags (e.g., --mri-input, --pdf-input, --simulate-peda)
- Auto-derives case ID from inputs
- Canonicalizes logs to <CASE>/applog/Logs
- NEW: Calls structure_guard.py --fix at the end to enforce final layout
"""
...
def resolve_norm_id(primary: Path, mri_input: str | None, mri_dir: str | None) -> str:
    ...
    for c in candidates:
        if c: return c
    raise ValueError("Unable to derive normalized ID (NNN_NN-NNN) from inputs. Provide a path containing the ID or supply --mri-input/--mri-dir.")
...
def id_matches(text: str, norm_id: str) -> bool:
    return extract_norm_id(text) == norm_id
...
def _autodetect_mri_input(case_dir: Path, norm_id: str) -> Path | None:
    ...
    if len(zpool) == 1: return zpool[0]
    if len(zpool) > 1:  return None
...
def build_mri_args(case_dir: Path, logs_root: Path, norm_id: str,
                   explicit_input: str | None, explicit_dir: str | None,
                   birthdate: str | None, apply_flag: bool,
                   strict_id: bool, logger: logging.Logger):
    meta = {'mri_input': None, 'source': None, 'id_mismatch': False, 'expected': norm_id, 'found': None}
    out_root = case_dir.parent
    if explicit_input:
        mri_input = Path(explicit_input); meta['source'] = 'explicit_input'
        ...
        mri_input = Path(explicit_dir); meta['source'] = 'explicit_dir'
        ...
        mri_input = _autodetect_mri_input(case_dir, norm_id); meta['source'] = 'autodetect'
    ...
    if apply_flag: args.append("--apply")
    return args, "", meta
...
def _autodetect_tdc_input(raw_case: Path, case_dir: Path, norm_id: str) -> Path | None:
    ...
    pool = list({p.resolve(): p for p in pool}.values())
    if len(pool) == 1: return pool[0]
    pool.sort(key=lambda p: p.stat().st_mtime, reverse=True)
    return pool[0]

def resolve_case_and_logs(raw_case: Path, out_root: str | None, norm_id: str, explicit_log_root: str | None):
    warn = None
...
def find_local_db(case_dir: Path, norm_id: str) -> Path | None:
    ...
    if not tdc_root.exists(): return None
...
def _normalize_pdf_suffix(name: str) -> str:
    if _PDF_EXT_RE.search(name): return _PDF_EXT_RE.sub(".pdf", name)
    if name.lower().endswith(".pdf") and not name.endswith(".pdf"): return name[:-4] + ".pdf"
    return name

def _score_pdf_candidate(p: Path, case_id: str) -> int:
    n = p.name.lower(); score = 0
    if case_id.lower() in n: score += 3
    if any(k in n for k in _PDF_KEYWORDS): score += 2
...
def _find_all_pdfs_multi(case_dir: Path, case_id: str):
    results: List[Path] = []
    if case_dir.exists():
...
def _best_pdf(pdfs, case_id: str):
    if not pdfs: return None
    return sorted(pdfs, key=lambda p: (_score_pdf_candidate(p, case_id), p.stat().st_mtime), reverse=True)[0]

def handle_case_pdf(case_dir: Path, norm_id: str, logger: logging.Logger, dry_run: bool,
                    dest_basename: str = "TreatmentReport", explicit_pdf: Optional[str] = None) -> None:
    if explicit_pdf:
        ...
        if not pdfs:
            logger.info("PDF: None found; skipping"); return
        cand = _best_pdf(pdfs, norm_id)
        if not cand:
            logger.info("PDF: No suitable candidate; skipping"); return
    ...
    if fixed_name != cand.name:
        ...
        if not dry_run:
            try:
                ...
                cand.rename(new_path); cand = new_path
    ...
    if dest.exists():
        ...
        while True:
            ...
            if not alt.exists(): dest = alt; break
            i += 1
    logger.info(f"PDF: → Target '{dest.name}'")
    if dry_run:
        logger.info("PDF: [DRY-RUN] Would move file"); return
    try:
...
def main():
    ap = argparse.ArgumentParser(description="TDC → MRI → PDF normalize → local.db anonymize → PEDA → archive orchestrator. (compat+autoid+guard)")
    ...
    ap.add_argument("--pdf-dest-name", default="TreatmentReport")
    ...
    if args.archive_only:
        ...
        if pedav and pedav.name.lower().endswith("-video"):
            logger.error("Refusing to archive '*-Video'"); sys.exit(2)
        if not pedav:
            logger.warning("ARCHIVE: No PEDAv* folder found"); sys.exit(0)
        label = args.peda_name or pedav.name
        if args.dry_run:
            logger.info(f"[DRY-RUN] Would archive {pedav}"); sys.exit(0)
        try:
            _archive_pedav_dir(pedav, case_dir, norm_id, label, logger, clean=args.clean_peda)
        except Exception as e:
            logger.error(f"ARCHIVE: Failed {e}"); sys.exit(1)
        sys.exit(0)
    if not args.skip_tdc:
        ...
        if tdc.exists():
            ...
            if args.simulate_tdc: tdc_args.append("--simulate")
            if tdc_input: tdc_args += ["--input", str(tdc_input)]
            if args.allow_id_mismatch: tdc_args.append("--allow-id-mismatch")
    ...
    if not args.skip_mri:
        ...
        if mri.exists():
            ...
            if mri_args is None:
                ...
                if args.simulate_mri: mri_args.append("--simulate")
    ...
    if args.anon_db:
        ...
        if not db_path:
            ...
            try:
                ...
                logger.error(f"ANON: Failed {e}"); sys.exit(1)
    else:
    ...
    # --- FINAL GUARD ENFORCEMENT ---
    guard = Path(__file__).parent / "structure_guard.py"
    if guard.exists():
        try:
            rc = run_py(logger, guard, [str(case_dir), "--id", norm_id, "--fix"], args.dry_run)
            if rc == 0:
                logger.info("GUARD: Final layout is canonical.")
            else:
                logger.warning(f"GUARD: Fix attempted; non-zero exit {rc}. Check console.")
        except Exception as e:
            logger.warning(f"GUARD: Could not run structure_guard.py ({e})")
    else:
        logger.info("GUARD: structure_guard.py not found; skipping final enforcement.")
